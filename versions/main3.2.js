...
function box(node, io){
    if (io == true){
        ...
        headBox.style.top = (`${node.y + node.radius - 60}px`)
        ...
        timeBox.style.left = (`${node.x + node.radius + 50}px`)
        timeBox.style.top = (`${node.y + node.radius - 22}px`)
        ...
        aTime.style.left = (`${node.x + node.radius + 50}px`)
        aTime.style.top = (`${node.y + node.radius + 192}px`)
        ...
        btnTime.style.left = (`${node.x + node.radius + 156}px`)
        btnTime.style.top = (`${node.y + node.radius + 192}px`)
        ...
        function additionalTime(node){
            ...
            node.time = node.time + (timeSplitArray[0] * 3600000) + (timeSplitArray[1] * 60000) + (timeSplitArray[2] * 1000);
            recurseTime(node);
            updateData(node);
        }
...
function addNode(id, x, y, radius, conns, time, name, text){
    n.push({
        id: id,
        x: x,
        y: y,
...
function clicked(){
    ...
    //creates new node
        ...
        n.push({id: n.length, io: false, x: x, y: y, conns: conns, colour: inactiveColour, hover: false, time: 0, timer: false, startTime: 0, name: 'Name Project: '});
...
function hover(){
    //is mouse over node
    for (let i = 0; i < n.length; i++){
        let rad = initRadius + n[i].time/5000;
